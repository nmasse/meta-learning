...
class Stimulus:
	...
	def get_tasks(self):
		...
		if par['task'] == 'go':
			...
			self.task_types = []
			for offset in [0, np.pi/4, np.pi, np.pi/2, 3*np.pi/4, -np.pi/4, -np.pi/2, -3*np.pi/4]:
	...
	def task_go(self, variant='go', offset=0, cat_boundary = 0):
		...
		if variant == 'go' or variant == 'go_OIC':
			stim_onset = np.random.randint(self.fix_time, self.fix_time+500, par['trials_per_seq'])//par['dt']
			stim_off = par['num_time_steps']
			fixation_end = np.ones(par['trials_per_seq'], dtype=np.int16)*(self.fix_time+500)//par['dt']
